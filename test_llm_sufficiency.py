...
async def test_llm_sufficiency():
    """测试LLM sufficiency assessment功能"""
    
    print("🧪 开始测试LLM sufficiency assessment...")
...
用户: 想做成魔法冒险主题的"""
    ...
    print("\n📝 测试1: 获取prompt模板...")
    try:
        prompt_template = prompts.get_sufficiency_assessment_prompt()
        print("✅ 获取prompt模板成功")
    ...
    try:
        ...
        print("✅ 格式化prompt成功")
        print(f"Prompt长度: {len(assessment_prompt)} 字符")
        print("Prompt开头:")
        print(assessment_prompt[:300] + "..." if len(assessment_prompt) > 300 else assessment_prompt)
    except Exception as e:
    ...
    try:
        response = await llm.apredict(assessment_prompt)
        ...
        try:
            ...
            print(f"❌ JSON解析失败: {je}")
            print(f"响应内容 (前500字符): {str(response)[:500]}")
    ...
    try:
        ...
        if hasattr(response, 'content'):
            ...
            try:
                ...
                print(f"❌ JSON解析失败: {je}")
                print(f"content内容 (前500字符): {str(content)[:500]}")
        else:
...
if __name__ == "__main__":
    asyncio.run(test_llm_sufficiency())